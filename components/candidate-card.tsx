...
export function CandidateCard({ c }: { c: Candidate }) {
  return (
    <div className="card group flex flex-col gap-4 p-5 transition hover:border-lime/40 hover:bg-black/[0.03]">
      <div className="flex items-start gap-3">
        <Avatar name={c.name} src={c.photo} size={52} />
        <div className="min-w-0 flex-1">
          <Link href={`/profile/${c.username}`} className="block truncate font-black tracking-tight group-hover:text-money">
            {c.name}
          </Link>
      ...
      {c.bio ? <p className="line-clamp-3 text-sm text-fg/75">{c.bio}</p> : null}
      ...
      <div className="mt-auto flex items-center justify-between gap-2 border-t border-line pt-4">
        <div>
          <div className="text-lg font-black tabular-nums text-money">{usd(c.current_bid)}</div>
